...
链路一全部为 Vite/Vue 标准能力，**无外部依赖风险**。工作量重心在 CSS 视觉实现（玻璃拟态、流光、动效），已由 PRD 视觉规范约束。建议先行完成设计 token（CSS Variables 主题层），再批量落组件。
...
| --- | --- | --- |
| V1.0 | monorepo 搭建 + 设计 token + 六个基础组件 + Showcase 骨架 | 一个组件（Button）完成：glob 注册 → 3 个变体 playground → 复制代码 → 锚点导航，全链路跑通 |
...
| 视觉打磨工作量超预期（玻璃拟态/动效调优） | 中 | 设计 token 先行；组件批次推进，每批次固化一次视觉基线 |
